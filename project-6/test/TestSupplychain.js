...
contract("SupplyChain", function(accounts) {
  ...
  it("Testing smart contract function buyItem() that allows a distributor to buy coffee", async () => {
    ...
    const farmWallet = web3.eth.getBalance(originFarmerID);
    ...
    const newFarmWallet = web3.eth.getBalance(originFarmerID);
    ...
    assert.equal(eventEmitted, true, "Invalid event emitted");
    assert.ok(
      farmWallet.plus(productPrice).eq(newFarmWallet),
  ...
  it("Testing smart contract function fetchItemBufferOne() that allows anyone to fetch item details from blockchain", async () => {
    ...
    assert.equal(resultBufferOne[0].toString(), "9", "Error: invalid sku");
    assert.equal(
      resultBufferOne[1].toString(),
      "1",
    ...
    assert.equal(
      resultBufferOne[3],
      originFarmerID,
      "Error: invalid origin farm id"
    );
    assert.equal(
      resultBufferOne[4],
      originFarmName,
      "Error: invalid origin farm name"
    );
    assert.equal(
      resultBufferOne[5],
      originFarmInformation,
      "Error: invalid origin farm info"
    );
    assert.equal(
      resultBufferOne[6],
      originFarmLatitude,
      "Error: invalid origin farm lat"
    );
    assert.equal(
      resultBufferOne[7],
      originFarmLongitude,
      "Error: invalid origin farm long"
    );
  });
  ...
  it("Testing smart contract function fetchItemBufferTwo() that allows anyone to fetch item details from blockchain", async () => {
    ...
    assert.equal(resultBufferTwo[0].toString(), "10", "Error: invalid sku");
    assert.equal(resultBufferTwo[1].toString(), "1", "Error: invalid upc");
    assert.equal(
      resultBufferTwo[2].toString(),
      "11",
      "Error: invalid product id"
    );
    assert.equal(
